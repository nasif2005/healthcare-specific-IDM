import json
import hashlib
from sympy import mod_inverse
from sympy import mod_inverse as sympy_mod_inverse
from bplib.bp import BpGroup
from petlib.pack import encode, decode
from Crypto.Hash import SHA256
from Crypto.Cipher import AES
from Crypto.Util.Padding import pad, unpad
from petlib.bn import Bn
import time
import numpy as np


n = 
phi_n = 
e = 65537
d = 
# n bit length = 3072

# Issuer's Public Key Components:
R_values = [, , , , , , , ]
S = 
Z = 

# Random Values:
v = 
r = 

# R0 is the first element in R_values, and R1 to R6 are the subsequent elements
R0 = R_values[0]  # R0 for the user's secret key
R_attributes = R_values[1:]  # R1 to R6 for attributes /*important*/

# Patient credential order
Patient_credential_ORDER = ['credential_id', 'did_patient', 'patient_id', 'biometric_data', 'issue_date', 'did_apc']


# User's private key
sk_user = 
pk_user = pow(R0, sk_user, n)

#print("User's private key:", sk_user)
#print("User's public key:", pk_user) 


file_path = r"/home/nmuslim162022/Desktop/mycode/patient_credential_signature.json"

# Load the JSON file
with open(file_path, 'r') as file:
    patient_credential = json.load(file)

#print ("patient_credential: ", patient_credential)
#print ("patient_credential['biometric_data']: ", patient_credential['info']['biometric_data'])



 # Function to hash and reduce an attribute
def hash_and_reduce(attribute, n):
    attribute_hash = hashlib.sha256(attribute.encode()).hexdigest()
    attribute_int = int(attribute_hash, 16)
    return attribute_int % n


# CORRECTED GENERATION FUNCTION
def proof_of_knowledge_patient_credentia_generation(disclosed_attributes_dict, patient_credential):
    # Map all patient attributes to m_i for internal use
    m = []
    for attr_name in Patient_credential_ORDER:
        value = patient_credential['info'][attr_name]
        attribute_value = hash_and_reduce(value, n)
        m.append(attribute_value)

    # Calculation of R_m
    R_m = 1
    for i, attr_value in enumerate(m):
        R_m = (R_m * pow(R_attributes[i], attr_value, n)) % n
    
    R = (R_m * pk_user) % n

    # Calculate A_prime and v_prime
    S_r = pow(S, r, n)
    S_r_inv = mod_inverse(S_r, n)
    A_prime = (patient_credential['signature']['A'] * S_r_inv) % n
    v_prime = (v + patient_credential['signature']['e'] * r)

    # Calculate Z_prime (the value the verifier will check against)
    Z_prime = (pow(A_prime, patient_credential['signature']['e'], n) * pow(S, v_prime, n) * R) % n
    #print("Z_prime (to be verified):", Z_prime)

    # Fixed random values for the proof
    e_tilde = 71283868338990786193364740118650178393
    v_tilde = 234672227276554471863123710545218028003
    sk_user_tilde = 213712384778698537675704610942567300823
    nonce = 237133831923579981081203483126789063270
    
    # Random values for hidden messages (m_i_tilde)
    m_i_tilde = {
        0: 114096786540045931831531287662410155747, 1: 336880516579979780763399956619176063555,
        2: 134364043404748232386372829164288157778, 4: 289347234987234987234987234987234987234,
        5: 198234987234987234987234987234987234987
    }

    # Determine hidden indices
    hidden_indices = []
    disclosed_keys = list(disclosed_attributes_dict.keys())
    for i, attr_name in enumerate(Patient_credential_ORDER):
        if attr_name not in disclosed_keys:
            hidden_indices.append(i)

    # Initial Z_tilde calculation
    Z_tilde = (pow(A_prime, e_tilde, n) * pow(S, v_tilde, n) * pow(R0, sk_user_tilde, n)) % n

    # Incorporate hidden attributes into Z_tilde
    for idx in hidden_indices:
        R_i = R_attributes[idx]
        Z_tilde = (Z_tilde * pow(R_i, m_i_tilde[idx], n)) % n
    
    # Calculate the challenge 'c'
    data_to_hash = str(A_prime) + str(Z_tilde) + str(nonce)
    c = int(hashlib.sha256(data_to_hash.encode()).hexdigest(), 16)

    # Calculate "hat" values
    e_hat = e_tilde + c * patient_credential['signature']['e']
    v_hat = v_tilde + c * v_prime
    sk_user_hat = sk_user_tilde + c * sk_user

    # Calculate m_hat for hidden attributes
    m_hat = {}
    for idx in hidden_indices:
        m_hat[str(idx)] = m_i_tilde[idx] + c * m[idx] # Use string key for JSON compatibility

    # Return all necessary parameters for verification
    proof_parameters = {
        'A_prime': A_prime,
        'Z_prime': Z_prime, # Now included for the verifier
        'Z_tilde': Z_tilde,
        'c': c,
        'e_hat': e_hat,
        'v_hat': v_hat,
        'sk_user_hat': sk_user_hat,
        'm_hat': m_hat,
        'disclosed_attributes': disclosed_attributes_dict, # Pass the dictionary
        'nonce': nonce
    }

    return proof_parameters





# Function to verify the proof of knowledge of the user's patient id

# FINAL CORRECTED VERIFICATION FUNCTION
def verify_proof_of_knowledge_patient_id(proof_of_knowledge_verification_parameters):
    """
    Verifies the proof of knowledge based on the CL signature scheme using direct equation comparison.
    """
    print("\n--- Verifying proof of knowledge ---")

    # 1. Extract all parameters from the proof
    A_prime = proof_of_knowledge_verification_parameters['A_prime']
    Z_prime = proof_of_knowledge_verification_parameters['Z_prime']
    Z_tilde = proof_of_knowledge_verification_parameters['Z_tilde']
    c = proof_of_knowledge_verification_parameters['c']
    e_hat = proof_of_knowledge_verification_parameters['e_hat']
    v_hat = proof_of_knowledge_verification_parameters['v_hat']
    sk_user_hat = proof_of_knowledge_verification_parameters['sk_user_hat']
    m_hat_hidden = proof_of_knowledge_verification_parameters['m_hat']
    disclosed_attributes = proof_of_knowledge_verification_parameters['disclosed_attributes']
    nonce = proof_of_knowledge_verification_parameters['nonce']

    # 2. As a first step, recompute the challenge hash to ensure Z_tilde wasn't tampered with.
    # This is a critical check.
    data_to_hash_verification = str(A_prime) + str(Z_tilde) + str(nonce)
    c_prime = int(hashlib.sha256(data_to_hash_verification.encode()).hexdigest(), 16)
    
    #print("\nOriginal challenge c:      ", c)
    #print("Recomputed challenge c':   ", c_prime)
    
    if c != c_prime:
        print("\nProof is INVALID! (Challenge hash does not match, proof was likely tampered with)")
        return False
    
    # 3. Calculate the LEFT-HAND-SIDE (LHS) of the verification equation
    # LHS = Z_tilde * (Z_prime)^c
    LHS = (Z_tilde * pow(Z_prime, c, n)) % n

    # 4. Calculate the RIGHT-HAND-SIDE (RHS) of the verification equation
    # RHS = (A')^e_hat * S^v_hat * R0^sk_user_hat * product(R_i^m_hat_i) for all i
    
    # Start with the base components using their "hat" values
    RHS = (pow(A_prime, e_hat, n) * pow(S, v_hat, n) * pow(R0, sk_user_hat, n)) % n

    # Multiply in the terms for the HIDDEN attributes using the m_hat values from the proof
    for idx_str, m_hat_value in m_hat_hidden.items():
        idx = int(idx_str)
        R_i = R_attributes[idx]
        RHS = (RHS * pow(R_i, m_hat_value, n)) % n
        
    # Multiply in the terms for the DISCLOSED attributes.
    # The verifier must re-calculate m_hat for these attributes.
    # m_hat_disclosed = m_tilde_disclosed + c * m_disclosed. Since we don't know m_tilde,
    # we use the property that R_i^m_hat = R_i^m_tilde * (R_i^m_i)^c.
    # The Z_tilde term on the LHS already contains all R_i^m_tilde terms.
    # So the RHS only needs the (R_i^m_i)^c part for disclosed attributes.
    for attr_name, attr_value in disclosed_attributes.items():
        idx = Patient_credential_ORDER.index(attr_name)
        m_i = hash_and_reduce(str(attr_value), n)
        R_i = R_attributes[idx]
        
        # Calculate (R_i^m_i)^c
        term_to_multiply = pow(pow(R_i, m_i, n), c, n)
        RHS = (RHS * term_to_multiply) % n

    #print(f"\nLHS (Z_tilde * Z_prime^c): {LHS}")
    #print(f"RHS (recomputed from hats): {RHS}")
    
    # 5. Final check
    if LHS == RHS:
        print("\nProof is VALID!")
        return True
    else:
        print("\nProof is INVALID! (LHS != RHS)")
        return False

 



num_iterations = 1 # 12
execution_times = []

for i in range(num_iterations):

    start_time = time.perf_counter_ns()

    ##### CORRECTED MAIN CALLING BLOCK
    # Computed by the patient

    # Define which attributes to disclose in a dictionary format
    disclosed_attribute = {
        'biometric_data': patient_credential['info']['biometric_data']
    }


    # Call the generation function with the DICTIONARY
    proof_params = proof_of_knowledge_patient_credentia_generation(
        disclosed_attribute, 
        patient_credential
    )


    #print ("proof_of_knowledge_patient_credential_verification_parameters: ", proof_params)


    file_path = r"/home/nmuslim162022/Desktop/mycode/signed_pseudonym_token.json"

    # Load the JSON file
    with open(file_path, 'r') as file:
        PT = json.load(file)

    #print ("PT_credential: ", PT)

    appointment_confirmation_code = "76b3"

    # Construct the credential with signature
    IdentityVerificationRequest = {
            'info': {
                "AppointmentConfirmationCode": appointment_confirmation_code,
                "biometric_data":patient_credential['info']['biometric_data'],
                "proof_of_knowledge_biometric_data_verification_parameters": proof_params,
                "PT": PT,
                "ScheduleInfo": "ScheduleInfo"
            },
            'signature': None
    }


    # It's crucial to serialize in a consistent way (sorted keys) so the signature is always verifiable.
    # The result must be bytes.
    serialized_payload = json.dumps(IdentityVerificationRequest['info'], sort_keys=True, separators=(',', ':')).encode('utf-8')

    # Typically, you sign the HASH of the data, not the raw data itself.
    payload_hash = hashlib.sha256(serialized_payload).digest()


    #print ("payload_hash: ", payload_hash)


    # Schnorr parameters (matching your earlier setup)
    p1 = 162259276829213363391578010288127
    q1 = 81129638414606681695789005144063
    g1_schnorr = 2

    def schnorr_signature_generate(msg_bytes, sk_sum):
        r = 123456789  # Use random in production!
        R = pow(g1_schnorr, r, p1)
        h = hashlib.sha256()
        h.update(R.to_bytes((p1.bit_length() + 7) // 8, 'big') + msg_bytes)
        c = int.from_bytes(h.digest(), 'big') % q1
        s = (r - c * sk_sum) % q1
        return s, c

    def schnorr_signature_verify(msg_bytes, s, c, Y_sum):
        R_prime = (pow(g1_schnorr, s, p1) * pow(Y_sum, c, p1)) % p1
        h = hashlib.sha256()
        h.update(R_prime.to_bytes((p1.bit_length() + 7) // 8, 'big') + msg_bytes)
        c_prime = int.from_bytes(h.digest(), 'big') % q1
        return c_prime == c


    # step : Sign message

    # Schnorr parameters (matching your earlier setup)
    p1 = 162259276829213363391578010288127
    q1 = 81129638414606681695789005144063
    g1_schnorr = 2

    file_path = "/home/nmuslim162022/Desktop/mycode/patient_pseudonym_data.json"

    def read_json_file(file_path):
        with open(file_path, "r") as json_file:
            return json.load(json_file)

    def group_element_to_hex(element):
        return encode(element).hex()

    def hex_to_group_element(hex_str, group):
        element_bytes = bytes.fromhex(hex_str)
        return decode(element_bytes)

    def group_element_to_scalar(elem):
        """Hash a group element to a scalar in Zq."""
        h = hashlib.sha256(encode(elem))
        scalar = int.from_bytes(h.digest(), 'big') % q1
        return scalar

    group = BpGroup()
    pseudonym_data = read_json_file(file_path)
    P_patient_a = hex_to_group_element(pseudonym_data['P_patient_a'], group)
    P_patient_b = hex_to_group_element(pseudonym_data['P_patient_b'], group)

    # Step 1: Hash group elements to scalars
    sk_a = group_element_to_scalar(P_patient_a)
    sk_b = group_element_to_scalar(P_patient_b)

    sk_schnorr = (sk_a + sk_b) % q1

    # Step 3: Public key for verification
    Y_schnorr = pow(g1_schnorr, sk_schnorr, p1)

    s, c = schnorr_signature_generate(payload_hash, sk_schnorr)
    """
    print("Signature (using sk_sum):")
    print("s:", s)
    print("c:", c)
    print("Y_sum:", Y_schnorr)
    """


    signature = {
            "c": c,
            "s": s,
            "Y_schnorr": Y_schnorr  # Directly convert Y_sign to hex since it's an integer
        }

    IdentityVerificationRequest['signature'] = signature

    #print ("IdentityVerificationRequest: ", IdentityVerificationRequest)

    ##############################################
    # computation by the Healthcare provider
    ##############################################

    # The healthcare organization verifies
    # 1. schnorr signature of the patient on the IdentityVerificationRequest based on the patient pseudonym
    # 2. appointment confirmation code
    # 3. proof of knowledge of the patient credential with disclosed attribute 'BioHash'
    # 4. schnorr signature on the PT by the PTA


    ### [1.0] begin the verification process of the schnorr signature of the patient
    def verify_appointment_confirmation_code (appointment_confirmation_code):
        return True

    appointment_confirmation_code = "777B"
    valid = verify_appointment_confirmation_code(appointment_confirmation_code)
    print ("valid appointment confirmation code")


    ### [2.0] begin the schnorr signature verification process of the patient on the IdentityVerificationRequest
    # It's crucial to serialize in a consistent way (sorted keys) so the signature is always verifiable.
    # The result must be bytes.
    serialized_payload = json.dumps(IdentityVerificationRequest['info'], sort_keys=True, separators=(',', ':')).encode('utf-8')

    # Typically, you sign the HASH of the data, not the raw data itself.
    payload_hash = hashlib.sha256(serialized_payload).digest()

    # Step: Verify schnorr signature on the IdentityVerificationRequest based on the patient pseudonym
    valid = schnorr_signature_verify(payload_hash, IdentityVerificationRequest['signature']['s'], IdentityVerificationRequest['signature']['c'], IdentityVerificationRequest['signature']['Y_schnorr'])
    #print("AppointmentScheduleRequest Signature valid?", valid)

    if IdentityVerificationRequest['info']['AppointmentConfirmationCode'] == "76b3":
        print ("AppointmentConfirmationCode is verified")


    ### [3.0] begin the verification process of the zk-proof of the disclosed attribute of the patient credential
    # verify the proof of knowledge of the patient credential (BioHash) 
    valid = verify_proof_of_knowledge_patient_id(proof_params) 
    #print(f"\nFinal verification result: {valid}")



    ### [4.0] begin the verification process of the PT

    #print ( "PT (zzz): ", IdentityVerificationRequest['info']['PT'])


    # Generate Schnorr signing key pair (PT)
    # sk_schnorr_pseudonym = 563452374                                 # Private key (example)               PT
    # Y_schnorr_pseudonym = pow(g1_schnorr, sk_schnorr_pseudonym, p1)  # Public key                          PT

    #print("\n--- Verifying the Pseudonym Token (PT) signature ---")


    # 1. Extract all components from the loaded PT JSON object
    P_patient_a = PT['info']['P_patient_a']
    P_patient_b = PT['info']['P_patient_b']
    rk_patient_to_HRR = PT['info']['rk_patient_to_HRR']
    encrypted_pid = PT['info']['encrypted_pid']

    s_from_token = PT['signature']['s']
    c_from_token = PT['signature']['c']
    Y_schnorr_pseudonym_PT  = PT['signature']['Y_sign']

    # 2. Reconstruct the exact byte sequence that was signed.
    # THE FIX: Concatenate the hex strings FIRST, then convert to bytes once.
    # This avoids any intermediate serialization metadata.
    full_hex_string = (
        P_patient_a +
        P_patient_b +
        rk_patient_to_HRR +
        encrypted_pid
    )
    pseudonym_token_bytes_to_verify = bytes.fromhex(full_hex_string)

    #print("\nReconstructed hex for PT verification:", pseudonym_token_bytes_to_verify.hex())
    #print("Public Key from Token (Y_sign):", Y_schnorr_pseudonym_PT)

    # 3. Verify the signature using the loaded components and the reconstructed bytes
    is_pt_signature_valid = schnorr_signature_verify(
        pseudonym_token_bytes_to_verify,
        s_from_token,
        c_from_token,
        Y_schnorr_pseudonym_PT
    )

    print("\nIs the Pseudonym Token signature valid?", is_pt_signature_valid)


    file_path = "/home/nmuslim162022/Desktop/mycode/hrr_key_data.json"

    def read_json_file(file_path):
        with open(file_path, "r") as json_file:
            return json.load(json_file)


    hrr_key_data = read_json_file(file_path)
    #print ("hrr_key_data: ", hrr_key_data)

    sk_HRR = Bn.from_hex(hrr_key_data['sk_HRR'])
    #pk_HRR = hrr_key_data['pk_HRR']


        # --- Helper Functions for PRE (Proxy Re-Encryption) ---
    # CORRECTED HELPER FUNCTION
    def derive_key(pid_enc_element):
        """Derive a symmetric key from pid_enc using SHA-256."""
        # THE FIX: Use petlib.pack.encode to match the key generation script.
        pid_enc_bytes = encode(pid_enc_element)
        hash_obj = SHA256.new(pid_enc_bytes)
        return hash_obj.digest()  # 32-byte symmetric key

    def decrypt_pid(encrypted_pid_bytes, key):
        """Decrypt the patient_id string using AES with the derived key."""
        iv = encrypted_pid_bytes[:16]
        ct = encrypted_pid_bytes[16:]
        cipher = AES.new(key, AES.MODE_CBC, iv)
        pt = unpad(cipher.decrypt(ct), AES.block_size)
        return pt.decode('utf-8')

    def healthcare_provider_computation(P_patient_a_elem, P_patient_b_elem, rk_patient_to_HRR_elem):
        """Re-encryption by the healthcare provider."""
        P_HRR_a = P_patient_a_elem
        P_HRR_b = group.pair(rk_patient_to_HRR_elem, P_patient_b_elem)
        return P_HRR_a, P_HRR_b

    def HRR_computation(P_HRR_a_elem, P_HRR_b_elem, encrypted_pid_bytes, sk_HRR_bn):
        """Decryption by HRR."""
        # This function now receives a Bn object for sk_HRR_bn.
        exponent = sk_HRR_bn.mod_inverse(group.order())
        PID_enc_decrypted = P_HRR_a_elem * (P_HRR_b_elem ** -exponent)
        symmetric_key_decrypted = derive_key(PID_enc_decrypted)
        patient_id_decrypted = decrypt_pid(encrypted_pid_bytes, symmetric_key_decrypted)
        return patient_id_decrypted

        # Deserialize all hex strings from the request back into their crypto object types.
    P_patient_a_elem = hex_to_group_element(P_patient_a, group)
    P_patient_b_elem = hex_to_group_element(P_patient_b, group)
    rk_patient_to_HRR_elem = hex_to_group_element(rk_patient_to_HRR, group)
    encrypted_pid_bytes = bytes.fromhex(encrypted_pid)

        # Now, call the functions with the correct object types.
    P_HRR_a, P_HRR_b = healthcare_provider_computation(P_patient_a_elem, P_patient_b_elem, rk_patient_to_HRR_elem)
    #print("Re-encrypted Pseudonym (P_HRR_a):", P_HRR_a)
    #print("Re-encrypted Pseudonym (P_HRR_b):", P_HRR_b)

        # Call the HRR function with the correct Bn object for the secret key.
    decrypted_patient_id = HRR_computation(P_HRR_a, P_HRR_b, encrypted_pid_bytes, sk_HRR)
    #print("\nDecrypted patient_id by HRR:", decrypted_patient_id)
    
    end_time = time.perf_counter_ns()
    exec_time_ms = (end_time - start_time) / 1_000_000
    execution_times.append(exec_time_ms)



# Exclude first and last execution times (reduce noise)
trimmed_times = execution_times[1:-1]
'''
if trimmed_times:
    avg_time = np.mean(trimmed_times)
    std_time = np.std(trimmed_times)
    print("Execution Times:", execution_times)
    print("Trimmed Execution Times:", trimmed_times)
    print(f"Average Execution Time: {avg_time:.2f} ms")
    print(f"Standard Deviation: {std_time:.2f} ms")
else:
    print("No valid execution times to calculate performance metrics.")

'''    